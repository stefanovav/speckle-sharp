...
public class ConverterAutocadCivil
{
  ...
  // Points
  public Point PointToSpeckle(AcadGeo.Point3d point, string units = null)
  {
  ...
  public Point PointToSpeckle(AcadGeo.Point2d point, string units = null)
  {
    //TODO: handle units.none?
    var u = units ?? ModelUnits;
    var extPt = ToExternalCoordinates(new AcadGeo.Point3d(point.X, point.Y, 0));
    return new Point(extPt.X, extPt.Y, extPt.Z, u);
  }

  public AcadGeo.Point3d PointToNative(Point point)
  {
    var _point = new AcadGeo.Point3d(
  ...
  public Point PointToSpeckle(AcadDB.DBPoint point, string units = null)
  {
    var u = units ?? ModelUnits;
    return PointToSpeckle(point.Position, u);
  }

  public AcadDB.DBPoint PointToNativeDB(Point point)
  {
    var _point = PointToNative(point);
    return new AcadDB.DBPoint(_point);
  }
  ...
  // Vectors
  public Vector VectorToSpeckle(AcadGeo.Vector3d vector, string units = null)
  {
    var u = units ?? ModelUnits;
  ...
  public AcadGeo.Vector3d VectorToNative(Vector vector)
  {
    var _vector = new AcadGeo.Vector3d(
  ...
  public AcadGeo.Matrix3d TransformToNativeMatrix(Transform transform)
  {
    // transform
    var scaledTransform = transform.ConvertToUnits(ModelUnits);
    AcadGeo.Matrix3d convertedTransform = new(scaledTransform);
    ...
    if (!convertedTransform.IsScaledOrtho())
      return new AcadGeo.Matrix3d(MakePerpendicular(convertedTransform));

    return convertedTransform;
  }

  // https://forums.autodesk.com/t5/net/set-blocktransform-values/m-p/6452121#M49479
  private static double[] MakePerpendicular(AcadGeo.Matrix3d matrix)
  {
    // Get the basis vectors of the matrix
    AcadGeo.Vector3d right = new(matrix[0, 0], matrix[1, 0], matrix[2, 0]);
    AcadGeo.Vector3d up = new(matrix[0, 1], matrix[1, 1], matrix[2, 1]);

    AcadGeo.Vector3d newForward = right.CrossProduct(up).GetNormal();
    ;

    AcadGeo.Vector3d newUp = newForward.CrossProduct(right).GetNormal();
  ...
  // Line
  public Line LineToSpeckle(AcadGeo.LineSegment2d line)
  {
  ...
  public Line LineToSpeckle(AcadGeo.Line3d line, string units = null)
  {
    var u = units ?? ModelUnits;
  ...
  public Line LineToSpeckle(AcadGeo.LineSegment3d line)
  {
    var _line = new Line(PointToSpeckle(line.StartPoint), PointToSpeckle(line.EndPoint), ModelUnits);
    _line.length = line.Length;
    _line.domain = IntervalToSpeckle(line.GetInterval());
  ...
  public AcadGeo.Line3d LineToNative(Line line)
  {
    var _line = new AcadGeo.Line3d(PointToNative(line.start), PointToNative(line.end));
    if (line.domain != null)
      try
      {
  ...
  // Rectangle
  public Polyline RectangleToSpeckle(AcadDB.Rectangle3d rectangle)
  {
    var _points = new List<AcadGeo.Point3d>
    {
      rectangle.LowerLeft,
  ...
  // Arc
  public Arc ArcToSpeckle(AcadGeo.CircularArc2d arc)
  {
    var interval = arc.GetInterval();

    // find arc plane (normal is in clockwise dir)
    var center3 = new AcadGeo.Point3d(arc.Center.X, arc.Center.Y, 0);
    AcadGeo.Plane plane = arc.IsClockWise
      ? new AcadGeo.Plane(center3, AcadGeo.Vector3d.ZAxis.MultiplyBy(-1))
      : new AcadGeo.Plane(center3, AcadGeo.Vector3d.ZAxis);
  ...
  public Arc ArcToSpeckle(AcadGeo.CircularArc3d arc)
  {
    var interval = arc.GetInterval();
    var _arc = new Arc(
  ...
  public AcadGeo.CircularArc3d ArcToNative(Arc arc)
  {
    var _arc = new AcadGeo.CircularArc3d(
  ...
  // Polyline
  private Polyline PolylineToSpeckle(AcadGeo.Point3dCollection points, bool closed)
  {
    double length = 0;
    List<AcadGeo.Point3d> vertices = new();
    foreach (AcadGeo.Point3d point in points)
    {
      if (vertices.Count != 0)
  ...
  public Polyline PolylineToSpeckle(AcadDB.Polyline3d polyline)
  {
    var points = new List<double>();
    ...
    if (polyline.IsNewObject)
      foreach (AcadGeo.Point3d vertex in polyline.Spline.NurbsData.GetControlPoints())
        points.AddRange(PointToSpeckle(vertex).ToList());
    // otherwise retrieve actual vertices from transaction
    else
      foreach (AcadDB.ObjectId id in polyline)
      {
        var vertex = (AcadDB.PolylineVertex3d)Trans.GetObject(id, AcadDB.OpenMode.ForRead);
  ...
  public AcadDB.Polyline3d PolylineToNativeDB(Polyline polyline)
  {
    var vertices = new AcadGeo.Point3dCollection();
    for (int i = 0; i < polyline.points.Count; i++)
      vertices.Add(PointToNative(polyline.points[i]));
    return new AcadDB.Polyline3d(AcadDB.Poly3dType.SimplePoly, vertices, polyline.closed);
  }

  // Polycurve
  public Polycurve PolycurveToSpeckle(AcadDB.Polyline2d polyline) // AC polyline2d can have linear, circlular, or elliptical segments
  {
    var polycurve = new Polycurve(ModelUnits) { closed = polyline.Closed };

    // extract segment curves
    var segments = new List<ICurve>();
    var exploded = new AcadDB.DBObjectCollection();
    polyline.Explode(exploded);
    AcadGeo.Point3d previousPoint = new();
    for (int i = 0; i < exploded.Count; i++)
    {
      ...
      if (i == 0 && exploded.Count > 1)
      {
        ...
        var connectionPoint = new AcadGeo.Point3d();
        ...
        segment = GetCorrectSegmentDirection(segment, connectionPoint, true, out AcadGeo.Point3d otherPoint);
      }
      else
      {
  ...
  public Polycurve PolycurveToSpeckle(AcadDB.Polyline polyline) // AC polylines are polycurves with linear or arc segments
  {
    ...
    for (int i = 0; i < polyline.NumberOfVertices; i++)
    {
      ...
      if (i == 0 && polyline.NumberOfVertices > 1)
      {
        ...
        if (nextSegment == null)
        {
          if (polyline.GetSegmentType(i + 1) == AcadDB.SegmentType.Point)
  ...
  private AcadGeo.Curve3d GetSegmentByType(AcadDB.Polyline polyline, int i)
  {
    AcadDB.SegmentType type = polyline.GetSegmentType(i);
    switch (type)
    {
      case AcadDB.SegmentType.Line:
        return polyline.GetLineSegmentAt(i);
      case AcadDB.SegmentType.Arc:
  ...
  private AcadDB.Curve GetCorrectSegmentDirection(
    ...
    AcadGeo.Point3d connectionPoint,
    bool isFirstSegment,
    out AcadGeo.Point3d nextPoint
  ...
  private AcadGeo.Curve3d GetCorrectSegmentDirection(
    AcadGeo.Curve3d segment,
    AcadGeo.Point3d connectionPoint,
    bool isFirstSegment,
    out AcadGeo.Point3d nextPoint
  ) // note sometimes curve3d may not have endpoints
  ...
  public AcadDB.Polyline PolycurveToNativeDB(Polycurve polycurve)
  {
    ...
    var plane = new AcadGeo.Plane(
      AcadGeo.Point3d.Origin,
      AcadGeo.Vector3d.ZAxis.TransformBy(Doc.Editor.CurrentUserCoordinateSystem)
    ); // TODO: check this
  ...
  private int BulgeDirection(Point start, Point mid, Point end)
  {
    ...
    double[] v1 = { end.x - start.x, end.y - start.y, end.z - start.z }; // vector from start to end point
    double[] v2 = { mid.x - start.x, mid.y - start.y, mid.z - start.z }; // vector from start to mid point
  ...
  // Spline
  public Curve SplineToSpeckle(AcadDB.Spline spline)
  {
    var curve = new Curve();

    // get nurbs and geo data
    var data = spline.NurbsData;
    var _spline = spline.GetGeCurve() as AcadGeo.NurbCurve3d;
    ...
    var points = data.GetControlPoints().OfType<AcadGeo.Point3d>().ToList();
    if (periodicClosed)
  ...
  public ApplicationObject PolycurveSplineToNativeDB(Polycurve polycurve)
  {
    ...
    AcadDB.Entity first = null;
    List<AcadDB.Entity> others = new();
    ...
    for (int i = 0; i < polycurve.segments.Count; i++)
    {
      ...
      {
        var newEntity =
          Trans.GetObject(modelSpaceRecord.Append(convertedItem), AcadDB.OpenMode.ForWrite) as AcadDB.Entity;
        appObj.Update(newEntity.Handle.ToString(), convertedItem: newEntity);
  ...
  // Curve
  // TODO: NOT TESTED
  public ICurve CurveToSpeckle(AcadGeo.Curve3d curve, string units = null)
  {
    var u = units ?? ModelUnits;
    ...
    switch (curve)
    {
      case AcadGeo.LineSegment3d line:
        return LineToSpeckle(line);
      case AcadGeo.CircularArc3d arc:
        return ArcToSpeckle(arc);
      default:
        return NurbsToSpeckle(curve as AcadGeo.NurbCurve3d);
    }
  }

  public ICurve CurveToSpeckle(AcadGeo.Curve2d curve, string units = null)
  {
    var u = units ?? ModelUnits;
    ...
    switch (curve)
    {
      case AcadGeo.LineSegment2d line:
        return LineToSpeckle(line);
      case AcadGeo.CircularArc2d arc:
        return ArcToSpeckle(arc);
      default:
        return NurbsToSpeckle(curve as AcadGeo.NurbCurve2d);
    }
  }

  public Curve NurbsToSpeckle(AcadGeo.NurbCurve2d curve)
  {
  ...
  public Curve NurbsToSpeckle(AcadGeo.NurbCurve3d curve)
  {
    var _curve = new Curve();
  ...
  public AcadGeo.NurbCurve3d NurbcurveToNative(Curve curve)
  {
    ...
    var points = new AcadGeo.Point3dCollection(_points.ToArray());
    ...
    var knots = new AcadGeo.KnotCollection();
    ...
    AcadGeo.DoubleCollection weights;
    weights =
      _weights.Distinct().Count() == 1
        ? new AcadGeo.DoubleCollection()
        : new AcadGeo.DoubleCollection(_weights.ToArray());

    AcadGeo.NurbCurve3d _curve = new(curve.degree, knots, points, weights, curve.periodic);
    if (curve.closed)
      _curve.MakeClosed();
  ...
  // Region
  public Mesh RegionToSpeckle(AcadDB.Region region, out List<string> notes, string units = null)
  {
    return GetMeshFromSolidOrSurface(out notes, region: region);
  }

  // Box
  public Box BoxToSpeckle(AcadGeo.BoundBlock2d bound)
  {
    ...
    var plane = new AcadGeo.Plane(
      new AcadGeo.Point3d(bound.GetMinimumPoint().X, bound.GetMinimumPoint().Y, 0),
      ucs.Xaxis,
      ucs.Yaxis
    );
  ...
  public Box BoxToSpeckle(AcadGeo.BoundBlock3d bound)
  {
    try
    {
      // convert min and max pts to speckle first
  ...
  public Box BoxToSpeckle(AcadDB.Extents3d extents)
  {
    try
    {
      // convert min and max pts to speckle first
  ...
  // Brep
  public Mesh SolidToSpeckle(AcadDB.Solid3d solid, out List<string> notes, string units = null)
  {
    return GetMeshFromSolidOrSurface(out notes, solid);
  }
  ...
  */
  ...
  public Mesh MeshToSpeckle(AcadDB.PolyFaceMesh mesh, string units = null)
  {
    ...
    foreach (AcadGeo.Point3d vert in _vertices)
  ...
  public Mesh MeshToSpeckle(AcadDB.SubDMesh mesh)
  {
    ...
    foreach (AcadGeo.Point3d vert in mesh.Vertices)
      vertices.AddRange(PointToSpeckle(vert).ToList());
  ...
  public AcadDB.PolyFaceMesh MeshToNativeDB(Mesh mesh)
  {
    ...
    using (AcadDB.Transaction tr = Doc.TransactionManager.StartTransaction())
    {
      ...
      AcadDB.BlockTableRecord btr = (AcadDB.BlockTableRecord)
        tr.GetObject(Doc.Database.CurrentSpaceId, AcadDB.OpenMode.ForWrite);
      ...
      {
        var vertex = new AcadDB.PolyFaceMeshVertex(points[i]);
      ...
      {
        AcadDB.FaceRecord face;
        if (mesh.faces[j] == 3) // triangle
        {
          face = new AcadDB.FaceRecord(
  ...
  private Mesh GetMeshFromSolidOrSurface(
    ...
    AcadDB.Solid3d solid = null,
    AcadDB.Surface surface = null,
    AcadDB.Region region = null
  )
  {
    Mesh mesh = null;
    double volume = 0;
