...
public class ConverterAutocadCivil
{
  ...
  private static AcadDB.LineWeight GetLineWeight(double weight)
  {
    ...
    var weights = Enum.GetValues(typeof(AcadDB.LineWeight)).Cast<int>().ToList();
    int closest = weights.Aggregate((x, y) => Math.Abs(x - hundredthMM) < Math.Abs(y - hundredthMM) ? x : y);
    return (AcadDB.LineWeight)closest;
  }

  public AcadDB.Entity DisplayStyleToNative(Base styleBase, AcadDB.Entity entity)
  {
    var color = new Color();
    if (styleBase is DisplayStyle style)
    {
      color = Color.FromArgb(style.color);
    ...
    {
      color = Color.FromArgb(material.diffuse);
      var alpha = (byte)(material.opacity * 255d);
      entity.Transparency = new Transparency(alpha);
    }
    entity.Color = Autodesk.AutoCAD.Colors.Color.FromRgb(color.R, color.G, color.B);

    return entity;
  }

  public DisplayStyle DisplayStyleToSpeckle(AcadDB.Entity entity)
  {
    ...
    switch (entity.Color.ColorMethod)
    {
      case ColorMethod.ByLayer:
        using (AcadDB.Transaction tr = Doc.Database.TransactionManager.StartTransaction())
        {
          ...
          {
            var layer = tr.GetObject(entity.LayerId, AcadDB.OpenMode.ForRead) as AcadDB.LayerTableRecord;
    ...
    if (entity.Linetype.ToUpper() == "BYLAYER")
      using (AcadDB.Transaction tr = Doc.Database.TransactionManager.StartTransaction())
      {
        ...
        {
          var layer = tr.GetObject(entity.LayerId, AcadDB.OpenMode.ForRead) as AcadDB.LayerTableRecord;
          var linetype = (AcadDB.LinetypeTableRecord)tr.GetObject(layer.LinetypeObjectId, AcadDB.OpenMode.ForRead);
    ...
    {
      case AcadDB.LineWeight.ByLayer:
        using (AcadDB.Transaction tr = Doc.Database.TransactionManager.StartTransaction())
        {
          if (entity.LayerId.IsValid)
          {
            var layer = tr.GetObject(entity.LayerId, AcadDB.OpenMode.ForRead) as AcadDB.LayerTableRecord;
            if (
              layer.LineWeight == AcadDB.LineWeight.ByLineWeightDefault || layer.LineWeight == AcadDB.LineWeight.ByBlock
            )
              lineWeight = (int)AcadDB.LineWeight.LineWeight025;
            else
        ...
        break;
      case AcadDB.LineWeight.ByBlock:
      case AcadDB.LineWeight.ByLineWeightDefault:
      case AcadDB.LineWeight.ByDIPs:
        lineWeight = (int)AcadDB.LineWeight.LineWeight025;
        break;
      default:
  ...
  // Hatches
  private HatchLoopType HatchLoopTypeToSpeckle(AcadDB.HatchLoopTypes type)
  {
    if (type.HasFlag(AcadDB.HatchLoopTypes.Outermost) || type.HasFlag(AcadDB.HatchLoopTypes.External))
      return HatchLoopType.Outer;
    if (type.HasFlag(AcadDB.HatchLoopTypes.Default))
      return HatchLoopType.Unknown;
    return HatchLoopType.Unknown;
  }

  private AcadDB.HatchLoopTypes HatchLoopTypeToNative(HatchLoopType type)
  {
    switch (type)
    {
      case HatchLoopType.Outer:
        return AcadDB.HatchLoopTypes.External;
      default:
        return AcadDB.HatchLoopTypes.Default;
    }
  }
  ...
  public ApplicationObject HatchToNativeDB(Hatch hatch)
  {
    ...
    AcadDB.BlockTableRecord modelSpaceRecord = Doc.Database.GetModelSpace();

    // convert curves
    var loops = new Dictionary<AcadDB.Curve, AcadDB.HatchLoopTypes>();
    if (hatch.loops != null)
      foreach (var loop in hatch.loops)
      {
        ...
        foreach (var convertedItem in converted)
        {
          ...
          if (curveId.IsValid)
          {
            AcadDB.HatchLoopTypes type = HatchLoopTypeToNative(loop.Type);
    ...
    switch (patternCategory)
    {
      case PatPatternCategory.kCustomdef:
        _hatch.SetHatchPattern(AcadDB.HatchPatternType.CustomDefined, hatch.pattern);
        break;
      case PatPatternCategory.kPredef:
      case PatPatternCategory.kISOdef:
        _hatch.SetHatchPattern(AcadDB.HatchPatternType.PreDefined, hatch.pattern);
        break;
      case PatPatternCategory.kUserdef:
        _hatch.SetHatchPattern(AcadDB.HatchPatternType.UserDefined, hatch.pattern);
        break;
      default:
        _hatch.SetHatchPattern(AcadDB.HatchPatternType.PreDefined, "SOLID");
        break;
    }
    ...
    if (style != null)
      _hatch.HatchStyle = Enum.TryParse(style, out AcadDB.HatchStyle hatchStyle)
        ? hatchStyle
        : AcadDB.HatchStyle.Normal;
    ...
    foreach (var entry in loops)
    {
      ...
      try
      {
        _hatch.AppendLoop(entry.Value, new AcadDB.ObjectIdCollection { entry.Key.ObjectId });
  ...
  private AcadDB.Polyline GetPolylineFromBulgeVertexCollection(AcadDB.BulgeVertexCollection bulges)
  {
    var polyline = new AcadDB.Polyline(bulges.Count);
    double totalBulge = 0;
    for (int i = 0; i < bulges.Count; i++)
    {
      AcadDB.BulgeVertex bulgeVertex = bulges[i];
  ...
  public BlockInstance BlockReferenceToSpeckle(AcadDB.BlockReference reference)
  {
    ...
    if (reference.IsDynamicBlock)
      btrObjId =
        reference.AnonymousBlockTableRecord != AcadDB.ObjectId.Null
          ? reference.AnonymousBlockTableRecord
          : reference.DynamicBlockTableRecord;

    var btr = (AcadDB.BlockTableRecord)Trans.GetObject(btrObjId, AcadDB.OpenMode.ForRead);
    definition = BlockRecordToSpeckle(btr);
    foreach (AcadDB.ObjectId id in reference.AttributeCollection)
    {
      AcadDB.AttributeReference attRef = (AcadDB.AttributeReference)Trans.GetObject(id, AcadDB.OpenMode.ForRead);
  ...
  public ApplicationObject InstanceToNativeDB(Instance instance, bool AppendToModelSpace = true)
  {
    ...
    if (ReceiveMode == ReceiveMode.Update)
    {
      ...
      try
      {
        ...
        {
          var existingObj = Trans.GetObject(existingObjId, AcadDB.OpenMode.ForWrite);
          existingObj.Erase();
        }
      }
      catch (Exception e)
    ...
    AcadDB.ObjectId definitionId = DefinitionToNativeDB(definition, out List<string> notes);
    if (notes.Count > 0)
      appObj.Update(log: notes);
    if (definitionId == AcadDB.ObjectId.Null)
    {
    ...
    AcadDB.BlockReference br = new(insertionPoint, definitionId);
  ...
  public BlockDefinition BlockRecordToSpeckle(AcadDB.BlockTableRecord record)
  {
    ...
    foreach (AcadDB.ObjectId id in record)
    {
      AcadDB.DBObject obj = Trans.GetObject(id, AcadDB.OpenMode.ForRead);
      AcadDB.Entity objEntity = obj as AcadDB.Entity;
  ...
  public AcadDB.ObjectId DefinitionToNativeDB(Base definition, out List<string> notes)
  {
    ...
    AcadDB.BlockTable blckTbl =
      Trans.GetObject(Doc.Database.BlockTableId, AcadDB.OpenMode.ForRead) as AcadDB.BlockTable;
    ...
    var bakedGeometry = new AcadDB.ObjectIdCollection(); // this is to contain block def geometry that is already added to doc space during conversion

    var converted = new List<AcadDB.Entity>();
    foreach (var item in conversionDict)
    {
      var geo = item.Key;
      var convertedGeo = new List<AcadDB.Entity>();
      ...
      switch (geo)
      {
        case Instance o:
          ...
          var instance = instanceAppObj.Converted.FirstOrDefault() as AcadDB.BlockReference;
          if (instance != null)
          {
      ...
      foreach (var convertedItem in convertedGeo)
        if (!convertedItem.IsNewObject && !(convertedItem is AcadDB.BlockReference))
    ...
    // create btr
    AcadDB.ObjectId blockId = AcadDB.ObjectId.Null;
    using (AcadDB.BlockTableRecord btr = new())
    {
  ...
  private string GetBlockDefName(AcadDB.BlockTableRecord btr)
  {
    ...
    if (btr.IsAnonymous || btr.IsDynamicBlock)
    {
      ...
      AcadDB.ObjectId referenceId = referenceIds.Count > 0 ? referenceIds[0] : AcadDB.ObjectId.Null;
      AcadDB.BlockReference reference =
        referenceId != AcadDB.ObjectId.Null
          ? Trans.GetObject(referenceId, AcadDB.OpenMode.ForRead) as AcadDB.BlockReference
          : null;
      if (reference == null)
        return fullName;
      ...
      if (btr.IsAnonymous)
      {
        AcadDB.BlockTableRecord dynamicBlock =
          reference.DynamicBlockTableRecord != AcadDB.ObjectId.Null
            ? Trans.GetObject(reference.DynamicBlockTableRecord, AcadDB.OpenMode.ForRead) as AcadDB.BlockTableRecord
            : null;
      ...
      foreach (AcadDB.DynamicBlockReferenceProperty prop in reference.DynamicBlockReferencePropertyCollection)
  ...
  private void ConvertWithDisplay(
    ...
    ref List<AcadDB.Entity> converted,
  ...
  )
  {
    ...
    foreach (AcadDB.Entity entity in convertedList)
      if (entity != null)
      {
        var displayedEntity =
  ...
  public Text TextToSpeckle(AcadDB.DBText text)
  {
    ...
    Base props = Utilities.GetApplicationProps(text, typeof(AcadDB.DBText), true, excludeProps);
  ...
  public Text TextToSpeckle(AcadDB.MText text)
  {
    ...
    Base props = Utilities.GetApplicationProps(text, typeof(AcadDB.MText), true, excludeProps);
  ...
  public AcadDB.Entity AcadTextToNative(Text text)
  {
    AcadDB.Entity _text = null;
    ...
    {
      case "MText":
        AcadDB.MText mText = TextToNative(text);
        mText.Location = PointToNative(textPosition);
        mText.TextStyleId = textStyle;
        Utilities.SetApplicationProps(mText, typeof(AcadDB.MText), sourceAppProps);
        _text = mText;
        break;
      case "DBText":
        var dbText = new AcadDB.DBText();
        ...
        Utilities.SetApplicationProps(dbText, typeof(AcadDB.DBText), sourceAppProps);
        _text = dbText;
        break;
      default:
  ...
  public AcadDB.MText TextToNative(Text text)
  {
    var _text = new AcadDB.MText();
  ...
  private AcadDB.ObjectId GetTextStyle(string styleName)
  {
    var textStyleTable =
      Trans.GetObject(Doc.Database.TextStyleTableId, AcadDB.OpenMode.ForRead) as AcadDB.DimStyleTable;
    foreach (AcadDB.ObjectId id in textStyleTable)
    {
      var textStyle = Trans.GetObject(id, AcadDB.OpenMode.ForRead) as AcadDB.DimStyleTableRecord;
  ...
  private Point3d GetTextCenter(AcadDB.Extents3d extents)
  {
  ...
  private Point3d GetTextCenter(AcadDB.DBText text)
  {
    ...
    switch (text.Justify)
    {
      case AcadDB.AttachmentPoint.BottomMid:
      case AcadDB.AttachmentPoint.BottomCenter:
        x = alignment.X;
        y = alignment.Y + height / 2;
        break;
      case AcadDB.AttachmentPoint.TopCenter:
      case AcadDB.AttachmentPoint.TopMid:
        x = alignment.X;
        y = alignment.Y - height / 2;
        break;
      case AcadDB.AttachmentPoint.MiddleRight:
        x = alignment.X - (alignment.X - position.X) / 2;
        y = alignment.Y;
        break;
      case AcadDB.AttachmentPoint.BottomRight:
        x = alignment.X - (alignment.X - position.X) / 2;
        y = alignment.Y + height / 2;
        break;
      case AcadDB.AttachmentPoint.TopRight:
        x = alignment.X - (alignment.X - position.X) / 2;
        y = alignment.Y - height / 2;
        break;
      case AcadDB.AttachmentPoint.MiddleCenter:
      case AcadDB.AttachmentPoint.MiddleMid:
        x = alignment.X;
        y = alignment.Y;
        break;
    }
  ...
  public Dimension DimensionToSpeckle(AcadDB.Dimension dimension)
  {
    ...
    switch (dimension)
    {
      case AcadDB.AlignedDimension o:
        ...
        props = Utilities.GetApplicationProps(o, typeof(AcadDB.AlignedDimension), true, ignore);
        _dimension = alignedDimension;
        break;
      case AcadDB.RotatedDimension o:
        ...
        props = Utilities.GetApplicationProps(o, typeof(AcadDB.RotatedDimension), true, ignore);
        _dimension = rotatedDimension;
        break;
      case AcadDB.OrdinateDimension o:
        ...
        props = Utilities.GetApplicationProps(o, typeof(AcadDB.OrdinateDimension), true, ignore);
        _dimension = ordinateDimension;
        break;
      case AcadDB.RadialDimension o:
        ...
        props = Utilities.GetApplicationProps(o, typeof(AcadDB.RadialDimension), true, ignore);
        _dimension = radialDimension;
        break;
      case AcadDB.DiametricDimension o:
        ...
        props = Utilities.GetApplicationProps(o, typeof(AcadDB.DiametricDimension), true, ignore);
        _dimension = diametricDimension;
        break;
      case AcadDB.ArcDimension o:
        ...
        props = Utilities.GetApplicationProps(o, typeof(AcadDB.ArcDimension), true, ignore);
        _dimension = arcDimension;
        break;
      case AcadDB.LineAngularDimension2 o:
        ...
        props = Utilities.GetApplicationProps(o, typeof(AcadDB.LineAngularDimension2), true, ignore);
        _dimension = lineAngularDimension;
        break;
      case AcadDB.Point3AngularDimension o:
        ...
        props = Utilities.GetApplicationProps(o, typeof(AcadDB.Point3AngularDimension), true, ignore);
    ...
    switch (className)
    {
      case "AlignedDimension":
        ...
        try
        {
          ...
          var alignedDimension = new AcadDB.AlignedDimension(
            alignedStart,
            alignedEnd,
            position,
            dimension.value,
          ...
          Utilities.SetApplicationProps(alignedDimension, typeof(AcadDB.AlignedDimension), sourceAppProps);
          _dimension = alignedDimension;
        }
        catch { }
        ;
      ...
      case "RotatedDimension":
        ...
        try
        {
          ...
          var rotatedDimension = new AcadDB.RotatedDimension(
            rotation,
            rotatedStart,
            rotatedEnd,
            position,
          ...
          Utilities.SetApplicationProps(rotatedDimension, typeof(AcadDB.RotatedDimension), sourceAppProps);
          _dimension = rotatedDimension;
        }
        catch { }
        break;
      case "OrdinateDimension":
        try
        {
          var ordinateDimension = DimensionToNative(dimension) as AcadDB.OrdinateDimension;
          if (ordinateDimension != null)
            Utilities.SetApplicationProps(ordinateDimension, typeof(AcadDB.OrdinateDimension), sourceAppProps);
          _dimension = ordinateDimension;
        }
        catch { }
        break;
      case "RadialDimension":
        try
        {
          var radialDimension = DimensionToNative(dimension) as AcadDB.RadialDimension;
          if (radialDimension != null)
            Utilities.SetApplicationProps(radialDimension, typeof(AcadDB.RadialDimension), sourceAppProps);
          _dimension = radialDimension;
        }
        catch { }
        break;
      case "DiametricDimension":
        ...
        try
        {
          ...
          var diametricDimension = new AcadDB.DiametricDimension(
            end,
            start,
            leaderLength,
            dimension.value,
          ...
          Utilities.SetApplicationProps(diametricDimension, typeof(AcadDB.DiametricDimension), sourceAppProps);
          _dimension = diametricDimension;
        }
        catch { }
        break;
      case "ArcDimension":
        ...
        try
        {
          ...
          var arcDimension = new AcadDB.ArcDimension(
            arc.Center,
            arc.StartPoint,
            arc.EndPoint,
            position,
          ...
          Utilities.SetApplicationProps(arcDimension, typeof(AcadDB.ArcDimension), sourceAppProps);
          _dimension = arcDimension;
        }
        catch { }
        break;
      case "LineAngularDimension2":
        try
        {
          var lineAngularDimension = DimensionToNative(dimension) as AcadDB.LineAngularDimension2;
          if (lineAngularDimension != null)
            Utilities.SetApplicationProps(lineAngularDimension, typeof(AcadDB.LineAngularDimension2), sourceAppProps);
          _dimension = lineAngularDimension;
        }
        catch { }
        break;
      case "Point3AngularDimension":
        try
        {
          var pointAngularDimension = DimensionToNative(dimension) as AcadDB.Point3AngularDimension;
          if (pointAngularDimension != null)
            Utilities.SetApplicationProps(pointAngularDimension, typeof(AcadDB.Point3AngularDimension), sourceAppProps);
          _dimension = pointAngularDimension;
        }
        catch { }
        break;
      ...
      case LengthDimension o:
        switch (o.measured)
        {
          case Arc a:
            ...
            try
            {
              var arcDimension = new AcadDB.ArcDimension(arcCenter, arcStart, arcEnd, position, dimension.value, style);
              _dimension = arcDimension;
            }
            catch { }
            break;
          case Line l:
            ...
            try
            {
              var radialDimension = new AcadDB.RadialDimension(
                radialStart,
                radialEnd,
                leaderLength,
                dimension.value,
      ...
      case AngleDimension o:
        try
        {
          ...
          if (Math.Round(line1Start.DistanceTo(line2Start), 3) == 0)
            _dimension = new AcadDB.Point3AngularDimension(
              line1Start,
              line1End,
              line2End,
              position,
            ...
            _dimension = new AcadDB.LineAngularDimension2(
              line1Start,
              line1End,
              line2Start,
              line2End,
      ...
      case DistanceDimension o:
        ...
        try
        {
          ...
          if (o.isOrdinate)
          {
            ...
            var ordinateDimension = new AcadDB.OrdinateDimension(useXAxis, end, position, dimension.value, style);
          ...
          else
          {
            ...
            if (dir.IsParallelTo(normal, Tolerance.Global))
              _dimension = new AcadDB.AlignedDimension(start, end, position, dimension.value, style);
            else
              _dimension = new AcadDB.RotatedDimension(angleBetween, start, end, position, dimension.value, style);
          }
        }
        catch { }
        break;
  ...
  private AcadDB.ObjectId GetDimensionStyle(string styleName)
  {
    var dimStyleTable = Trans.GetObject(Doc.Database.DimStyleTableId, AcadDB.OpenMode.ForRead) as AcadDB.DimStyleTable;
    foreach (AcadDB.ObjectId id in dimStyleTable)
    {
      var dimStyle = Trans.GetObject(id, AcadDB.OpenMode.ForRead) as AcadDB.DimStyleTableRecord;
  ...
  // Proxy Entity
  public Base ProxyEntityToSpeckle(AcadDB.ProxyEntity proxy)
  {
    ...
    var props = Utilities.GetApplicationProps(proxy, typeof(AcadDB.ProxyEntity));
